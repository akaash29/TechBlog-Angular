...
(function () {
  ...
  /* ---------- boot ----------------------------------------
     Runs once per full page load in the original static template
     (wired to DOMContentLoaded there). In an SPA shell, a route
     change swaps the DOM without firing DOMContentLoaded again, so
     this is exposed as LW.boot() and re-invoked by the app after
     every client-side navigation. Keep every step safe to repeat:
     re-query rather than assume nothing exists yet, and guard any
     one-time DOM insertion (the mobile drawer) so it isn't rebuilt
     on top of itself when the host element persists across routes.
     ------------------------------------------------------------ */
  LW.boot = function () {
    ...
    /* mobile drawer \u2014 build it once per .side element */
    var side = LW.$('.side');
    if (side && !side.dataset.lwDrawer) {
      side.dataset.lwDrawer = '1';
